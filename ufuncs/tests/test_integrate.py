...
import sys
sys.path.append(".")
from integrate import auto_integrate


def test_function1():
    function = lambda x: np.exp(-x)
    
    result = auto_integrate(function, Ninit=1, rtol=1.0e-15)
    np.testing.assert_allclose(result, 1., rtol=1.0e-15)
        
    result = auto_integrate(function, Ninit=10, rtol=1.0e-10)
    np.testing.assert_allclose(result, 1., rtol=1.0e-10)

    result = auto_integrate(function, Ninit=5, rtol=1.0e-08)
