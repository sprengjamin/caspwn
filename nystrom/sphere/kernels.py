...
@njit("UniTuple(float64, 4)(float64, float64, float64, float64, float64, float64, float64, float64, float64, int64, float64[:], float64[:])", cache=True)
def kernel_polar_finite(R, L, r, sign, K, k1, k2, phi, n, lmax, mie_a, mie_b):
    r"""
    ...
    e = math.exp(phase(R, L, r, K, k1, k2, phi))
    A, B, C, D = ABCD(K, k1, k2, phi)
    norm = 2 * math.pi * math.sqrt(k1 * k2) / (K * math.sqrt(kappa1 * kappa2))
    S1, S2 = S1S2_finite(K * R, z, n, lmax, mie_a, mie_b, True)
...
@njit("UniTuple(float64, 2)(float64, float64, float64, float64, float64, float64, float64, string, int64)", cache=True)
def kernel_polar_zero(R, L, r, k1, k2, phi, alpha, materialclass, lmax):
    r"""
    Returns the kernel of the reflection operator on a sphere in polar
    coordinates in symmetrized from.
    ...
    if phi == np.pi:
        return 0., 0.
    x = 2 * R * math.sqrt(k1 * k2) * abs(math.cos(phi / 2))
    e = math.exp(x - (k1 + k2) * (R + r * L))
    norm = 2 * math.pi * R
